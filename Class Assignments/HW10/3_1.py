...
def compute_error(train_data, train_labels):
    error_classes = np.zeros((10))
    ...
    for i in range(0, 10):
        compressed_images = np.matmul(
            train_data_classes[i], eig_vecs_classes[i])
        reconstructed_images = np.matmul(
            compressed_images, eig_vecs_classes[i].T)
        error = np.sum(
            (train_data_classes[i] - reconstructed_images)**2, axis=1)
        error = np.sqrt(error)
        error = np.mean(error)
        error_classes[i] = error

    return error_classes


error = compute_error(train_data, train_labels)
x = list(range(0, 10))
plt.bar(x, error)
